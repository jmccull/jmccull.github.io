...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def p():
        ...
        ax = plt.gca()
        ax.text(0.75,1.115,'$|\Delta \overline{z}| = $'+'{0:.4f}'.format(np.abs(meanz-meanz_cut)),color='gray', #0.65,0.85
                fontsize=18,transform=ax.transAxes)
        ax.text(0.75, 1.05, 'MAG_GAAP_Z Wide < ' + '{0:.2f}'.format(input.magcut()), color='gray', #0.541, 0.9
                fontsize=18, transform=ax.transAxes)
